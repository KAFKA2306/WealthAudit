    ...
    *,
    ...
    for branch in sorted(branches, key=lambda item: item.name):
        if branch.name == default_branch:
            decisions.append(Decision(branch.name, branch.sha, "keep", "default_branch"))
            continue
        if branch.protected:
            decisions.append(Decision(branch.name, branch.sha, "keep", "protected"))
            continue
        if not branch.name.startswith(prefixes):
            decisions.append(Decision(branch.name, branch.sha, "keep", "non_ephemeral_prefix"))
            continue
        if branch.name in open_refs:
            decisions.append(Decision(branch.name, branch.sha, "keep", "open_pull_request"))
            continue
        if branch.sha == default_sha:
            decisions.append(Decision(branch.name, branch.sha, "delete", "same_as_default"))
            continue
        if branch.sha in merged_heads.get(branch.name, set()):
            decisions.append(Decision(branch.name, branch.sha, "delete", "exact_merged_pr_head"))
            continue
        decisions.append(Decision(branch.name, branch.sha, "keep", "no_merge_proof"))
    return decisions
...
def apply_ancestry_proof(
    ...
    for decision in decisions:
        if decision.reason == "no_merge_proof" and decision.sha in ancestor_shas:
            upgraded.append(replace(decision, action="delete", reason="ancestor_of_default"))
        else:
...
def run(*, apply: bool, repo: str, token: str) -> int:
    ...
    unresolved_shas = {
        decision.sha for decision in decisions if decision.reason == "no_merge_proof"
    }
